@php
    $nav_menu = App\Helper\AdditionalResources::ClientNavigationMenu();
    $nav_child_menu = fn($id, $type = 'parent_id') => App\Helper\AdditionalResources::ClientNavigationMenu($id, $type);
    $route = fn($name) => Route::has($name) ? Route($name) : $name;
@endphp
...
<section>
      ...
      >
        <div class="container w-full flex items-center justify-between">
          <!-- Logo Image -->
          <a href="{{ Route('frontend.home') }}" class="block">
            <img
              src="{{ asset($additional_resource('logo')) }}"
              alt="Logo"
          ...
          @if ($nav_menu['status'])
            ...
            @foreach ($nav_menu['data'] as $menu)
              <li class="group relative 2xl:h-full flex items-start gap-4 2xl:gap-0 2xl:items-center justify-center" onclick="childMenu(event)">
                <a
                  class="font-medium capitalize text-lg font-oswald text-slate-600 group-hover:drop-shadow-primary group-hover:text-primary group-[.active-nav]:text-primary group-[.active-nav]:drop-shadow-primary flex gap-1 items-center group-hover:items-end justify-center duration-500"
                  href="{{ $route($menu->route) }}"
                >
                  <span class="capitalize">{{ $menu->name }}</span>

                  @if (count($nav_child_menu($menu->id)) > 0)
                    <span class="fill-slate-600 group-hover:fill-primary group-hover:drop-shadow-primary group-[.active-nav]:fill-primary group-[.active-nav]:drop-shadow-primary group-[.active-nav]:rotate-180 duration-500 group-hover:rotate-180">
                ...
                @if ($child_menus = $nav_child_menu($menu->id)) 
                  <ul class="hidden group-[.active-nav]:block group-hover:block 2xl:absolute top-20 left-0 font-oswald bg-primary text-slate-200 text-sm font-medium">
                    ...
                    @foreach ($child_menus as $child_menu)
                      <li class="duration-300 relative px-6 py-3 hover:bg-secondary group groupchild">
                        <a href="{{ $route($child_menu->route) }}" class="font-medium capitalize text-md font-oswald text-slate-100 flex gap-1 items-start justify-start duration-500 groupchild group-[child.active-nav]:bg-secondary">
                          <span class="capitalize text-sm text-nowrap">{{ $child_menu->name }}</span>

                          @if (count($nav_child_menu($child_menu->id, 'child_id')) > 0)
                            <span class="fill-slate-100 group-[child:hover]:-rotate-90 rotate-90 duration-300">
                              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" width="18" height="18">
                                <path d="M41 288h238c21.4 0 32.1 25.9 17 41L177 448c-9.4 9.4-24.6 9.4-33.9 0L24 329c-15.1-15.1-4.4-41 17-41z"></path>
                              </svg>
                            </span>
                        ...
                        <!-- Children -->
                        @if ($children_menus = $nav_child_menu($child_menu->id, 'child_id'))
                          <ul class="hidden group-[child.active-child-nav]:block group-[child:hover]:block 2xl:absolute top-0 left-32 m-3 2xl:my-0 2xl:mr-0 2xl:ml-3.5 font-oswald bg-primary text-slate-200 text-sm font-medium">
                            @foreach ($children_menus as $children_menu)
                                <li class="duration-300">
                                  <a href="{{ $route($children_menu->route) }}" class="h-full block text-nowrap px-6 py-3 duration-500 hover:bg-secondary hover:text-slate-100">{{ $children_menu->name }}</a>
                                </li> 
                            @endforeach
                          </ul>
                        @endif
            ...
            @endforeach
          @else
            <li class="font-medium capitalize text-lg font-oswald text-red-500 group-hover:drop-shadow-primary group-hover:text-primary group-[.active-nav]:text-primary group-[.active-nav]:drop-shadow-primary flex gap-1 items-center group-hover:items-end justify-center duration-500">{{ $nav_menu['message'] }}</li>
          @endif
